...
<!--
    ...
    function inicio()
    {
        ...
        {
            $url = '<?php echo site_url('maestro/searchEAN'); ?>';
            $.post( $url, { ean: $ean }).done(function( data ) {
                
                if(data == 1)
                {
                    $('#ean_message').html("Ya existe este codigo de barras dado de alta.").css('color', 'red');
                }else{
                    $('#ean_message').html("Ok, Este codigo de barras no esta dado de alta.").css('color', 'green');
                }
        ...
        function guarda(event)
        {
            ...
            $url = '<?php echo site_url('maestro/saveProducto'); ?>';
