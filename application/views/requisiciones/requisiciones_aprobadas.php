            ...
            <?php
            	foreach($requisicion->result() as $req){
                   ...
                   }elseif($req->estatus == 2){
                    $link1 = anchor('requisiciones/aplicar_pago_requisicion/'.$req->requisicion, 'Aplicar pago', array('class' => 'aplicar_pago_requisicion'));
                    $link2 = anchor('requisiciones/detalle_requisicion/'.$req->requisicion, 'Previo');
                    $link3 = null;#anchor('requisiciones/rechazar_requisicion/'.$req->requisicion, 'Rechazar', array('class' => 'rechazar_requisicion'));
                   }else{
                    $link1 = null;#anchor('requisiciones/aplicar_pago_requisicion/'.$req->requisicion, 'Aplicar pago', array('class' => 'aplicar_pago_requisicion'));
                    $link2 = anchor('requisiciones/detalle_requisicion/'.$req->requisicion, 'Previo');
                    $link3 = null;#anchor('requisiciones/rechazar_requisicion/'.$req->requisicion, 'Rechazar', array('class' => 'rechazar_requisicion'));
                   }
            ?>
                                    <tr>
                                        ...
                                        <td><?php echo $req->aprobada; ?></td>
                                        <td><?php echo $req->id; ?></td>
                                        <td><?php echo $req->estatusAlmacen; ?></td>
